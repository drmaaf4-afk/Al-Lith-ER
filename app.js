...
function updateTitles(){
  ...
  if(pageTitle) pageTitle.textContent = `Emergency Department Planner - ${getCurrentMonthLabel()}`;
}
...
async function exportExcel(){
  ...
  const rosterTable = [['Day', 'Shift Time', 'Doctors Booked']];
  const onCallTable = [['Day', 'Shift Time', 'On-Call Doctors']];
  const totals = {};
  const days = getCurrentDaysArray();

  for(const day of days){
    let firstRosterRowForDay = true;
    let firstOnCallRowForDay = true;
    ...
    TIMES.forEach(time => {
      ...
      const onCallNames = onCall.map(name => emailToName[name] || name);
      ...
      if(bookedNames.length){
        rosterTable.push([
          firstRosterRowForDay ? `${getCurrentMonthName()} ${day}` : '',
          time,
          bookedNames.join(', ')
        ]);
        firstRosterRowForDay = false;
      }

      if(onCallNames.length){
        onCallTable.push([
          firstOnCallRowForDay ? `${getCurrentMonthName()} ${day}` : '',
          time,
          onCallNames.join(', ')
        ]);
        firstOnCallRowForDay = false;
      }
    });
  }

  if(rosterTable.length === 1){
    rosterTable.push(['—', '—', 'No booked shifts']);
  }

  if(onCallTable.length === 1){
    onCallTable.push(['—', '—', 'No on-call duties']);
  }
  ...
  if(totalsTable.length === 1){
    totalsTable.push(['—', 0, 0]);
  }

  const wsRoster = XLSX.utils.aoa_to_sheet(rosterTable);
  const wsOnCall = XLSX.utils.aoa_to_sheet(onCallTable);
  const wsTotals = XLSX.utils.aoa_to_sheet(totalsTable);

  const buildColWidths = table => table[0].map((_, colIndex) => {
    let maxLength = 10;
    table.forEach(row => {
      const cell = row[colIndex] ? row[colIndex].toString() : '';
      if(cell.length > maxLength) maxLength = cell.length;
    });
    return { wch: Math.min(maxLength + 3, 80) };
  });

  wsRoster['!cols'] = buildColWidths(rosterTable);
  wsOnCall['!cols'] = buildColWidths(onCallTable);
  wsTotals['!cols'] = buildColWidths(totalsTable);

  function hideUnused(ws, colsCount, rowsCount){
    const maxRows = 1048576;
    const maxCols = 16384; // Excel max columns = XFD

    const rows = [];
    for(let i = 0; i < rowsCount; i++){
      rows.push({});
    }
    for(let i = rowsCount; i < maxRows; i++){
      rows.push({ hidden: true });
    }
    ws['!rows'] = rows;
    ...
    for(let i = colsCount; i < maxCols; i++){
      cols.push({ hidden: true });
    }
    ws['!cols'] = cols;
  }

  hideUnused(wsRoster, 3, rosterTable.length);
  hideUnused(wsOnCall, 3, onCallTable.length);
  hideUnused(wsTotals, 3, totalsTable.length);

  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, wsRoster, 'Duty Roster');
  XLSX.utils.book_append_sheet(wb, wsOnCall, 'On Call Duties');
  XLSX.utils.book_append_sheet(wb, wsTotals, 'Doctor Totals');

  XLSX.writeFile(wb, `Emergency_Department_Planner_${getCurrentMonthName()}_${selectedYear}.xlsx`);
