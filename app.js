...
async function signup(){
  ...
  try{
    ...
    if(!doctorRow.email){
      const msg = 'This doctor has no email in doctors table.';
      setAuthStatus(msg, true);
      showToast(msg, 'err');
      return;
    }
    ...
    const { error } = await supabaseClient.auth.signUp({
      email: normalizeEmail(doctorRow.email),
      password
    });
...
async function exportExcel(){
  ...
  (doctors || []).forEach(d => {
    ...
    if(email) emailToName[email] = d.name;
  });
  ...
  for(const day of days){
    TIMES.forEach((time, timeIndex) => {
      ...
      const bookedNames = rows.map(r => {
        const value = emailToName[normalizeEmail(r.doctor)] || r.doctor;
        let label = value;
      ...
      const onCallNames = onCall.map(name => emailToName[normalizeEmail(name)] || name);

      for(const r of rows){
        const name = emailToName[normalizeEmail(r.doctor)] || r.doctor;
        if(!totals[name]){
      ...
      rosterTable.push([
        timeIndex === 0 ? ${getCurrentMonthName()} ${day} : '',
        time,
        bookedNames.length ? bookedNames.join(', ') : '—'
      ]);

      onCallTable.push([
        timeIndex === 0 ? ${getCurrentMonthName()} ${day} : '',
        time,
        onCallNames.length ? onCallNames.join(', ') : '—'
      ]);
    });
  }
  ...
  XLSX.writeFile(
    wb,
    Emergency_Department_Planner_${getCurrentMonthName()}_${selectedYear}.xlsx
  );
