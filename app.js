...
function getShiftOnCall(day, time){
  const rows = getShiftRows(day, time);
  const onCallRow = rows.find(row => row.on_call);
  return onCallRow ? onCallRow.on_call : '';
}
...
async function setOnCallDoctor(){
  ...
  if(clearError){
    console.error(clearError);
    const msg = 'Could not clear old on-call doctor.';
    setAdminStatus(msg, true);
    showToast(msg, 'err');
    return;
  }
  ...
  const msg = `${doctorName} is now on call for ${getCurrentMonthName()} ${day} ${time}.`;
  setAdminStatus(msg);
  showToast(msg, 'ok');
  await loadBookings();
}
...
async function clearOnCallDoctor(){
  ...
  if(error){
    console.error(error);
    const msg = 'Could not clear on-call doctor.';
    setAdminStatus(msg, true);
    showToast(msg, 'err');
    return;
  }

  const msg = `On-call doctor cleared for ${getCurrentMonthName()} ${day} ${time}.`;
  setAdminStatus(msg);
  showToast(msg, 'info');
  await loadBookings();
}
  ...
  for(const day of days){
    ...
    for(const time of TIMES){
      ...
      const names = rows.map(r => {
        ...
        if(r.doctor === onCall) label += ' (On Call)';
        return label;
      });
