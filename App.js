...
let selectedYear = Number(localStorage.getItem('selectedYear') || defaultDate.getFullYear());
let selectedMonthIndex = Number(localStorage.getItem('selectedMonthIndex') || defaultDate.getMonth());

const isSavedMonthAllowed = MONTH_OPTIONS.some(
  m => m.year === selectedYear && m.monthIndex === selectedMonthIndex
);

if(!isSavedMonthAllowed){
  selectedYear = MONTH_OPTIONS[0].year;
  selectedMonthIndex = MONTH_OPTIONS[0].monthIndex;
}
